 ...
 * - Two series: new users + cumulative total
 * - RTL: axes reversed
 */
const UserGrowthChart = ({ data, height = 260 }: UserGrowthChartProps) => {
  const { isDark } = useTheme();
  const [ready, setReady] = useState(false);

  useEffect(() => {
    // Wait one frame for CSS vars to settle
  ...
  const option = useMemo(() => {
    ...
    return {
      ...
      series: data.series.map((s, idx) => ({
        ...
        showSymbol: false,
        lineStyle: { width: 2.5 },
        areaStyle: idx === 0 ? { color: buildAreaGradient(CHART_COLORS.orange) } : undefined,
        data: s.data,
        emphasis: {
          focus: 'series',
