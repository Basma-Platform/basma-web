...
// ✅ تعريف نوع عنصر القائمة مع badge اختياري - استخدام ReactNode بدلاً من JSX.Element
interface NavItem {
  icon: ReactNode;
  label: string;
  path: string;
...
const DashboardSidebar = ({ isOpen, onClose, isMobile = false }: DashboardSidebarProps) => {
  ...
  const userNavItems: NavItem[] = [
    ...
    { icon: <FaBullhorn />, label: 'إعلاناتي', path: '/dashboard/my-announcements', isActive: location.pathname === '/dashboard/my-announcements' },
    ...
    { icon: <FaComment />, label: 'تعليقاتي', path: '/dashboard/my-comments', isActive: location.pathname === '/dashboard/my-comments' },
    { icon: <FaStar />, label: 'تقييماتي', path: '/dashboard/my-ratings', isActive: location.pathname === '/dashboard/my-ratings' },
  ];

  // ✅ روابط المستخدم الموثق (يضاف له روابط إضافية)
  const verifiedNavItems: NavItem[] = [
    { icon: <FaHome />, label: 'لوحة التحكم', path: '/dashboard', isActive: location.pathname === '/dashboard' },
    { icon: <FaBullhorn />, label: 'إعلاناتي', path: '/dashboard/my-announcements', isActive: location.pathname === '/dashboard/my-announcements' },
    { icon: <FaPlus />, label: 'إضافة إعلان', path: '/dashboard/create-announcement', isActive: location.pathname === '/dashboard/create-announcement' },
    { icon: <FaComment />, label: 'تعليقاتي', path: '/dashboard/my-comments', isActive: location.pathname === '/dashboard/my-comments' },
    { icon: <FaStar />, label: 'تقييماتي', path: '/dashboard/my-ratings', isActive: location.pathname === '/dashboard/my-ratings' },
    { icon: <FaUserCheck />, label: 'موثق ✓', path: '/dashboard/verified-badge', isActive: location.pathname === '/dashboard/verified-badge', badge: '✓', badgeColor: '#28A745' },
  ];

  // ✅ روابط مشتركة للجميع (تظهر في الأسفل)
  ...
  // ✅ اختيار الروابط حسب نوع المستخدم
  let mainNavItems: NavItem[];
  if (isAdmin) {
    mainNavItems = adminNavItems;
  } else if (isVerified) {
    mainNavItems = verifiedNavItems;
  } else {
    mainNavItems = userNavItems;
  }

  // ✅ دمج الروابط
  const navItems = [...mainNavItems, ...sharedNavItems];
    ...
    >
      ...
      <div style={{ padding: '20px 24px 16px', borderBottom: '1px solid var(--border-color)', display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexShrink: 0 }}>
        <Link to="/" style={{ display: 'flex', alignItems: 'center', gap: '10px', textDecoration: 'none' }}>
          ...
          {isVerified && !isAdmin && <span style={{ backgroundColor: '#28A745', color: '#FFFFFF', fontSize: '0.5rem', padding: '2px 8px', borderRadius: '10px', fontWeight: 600, marginRight: '4px' }}>موثق</span>}
        </Link>
        {isMobile && (
      ...
      <div style={{ padding: '16px 20px', borderBottom: '1px solid var(--border-color)', display: 'flex', alignItems: 'center', gap: '12px', flexShrink: 0 }}>
        ...
        <div style={{ flex: 1, minWidth: 0 }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: '6px', flexWrap: 'wrap' }}>
            ...
            {isVerified && (
              <span style={{ color: '#28A745', fontSize: '0.55rem', display: 'flex', alignItems: 'center', gap: '2px', backgroundColor: 'rgba(40,167,69,0.1)', padding: '1px 8px', borderRadius: '10px' }}>
        ...
        {navItems.map((item, index) => (
          <motion.div key={index} custom={index} initial="hidden" animate="visible" variants={itemVariants}>
            ...
            >
              ...
              {item.badge && (
                <span style={{ 
                  backgroundColor: item.badgeColor || '#E87A20', 
                  color: '#FFFFFF', 
                  fontSize: item.badge === '✓' ? '0.6rem' : '0.6rem', 
                  padding: item.badge === '✓' ? '1px 6px' : '1px 8px', 
                  borderRadius: '12px', 
                  fontWeight: 600, 
                  minWidth: '20px', 
                  textAlign: 'center' 
